...
window.__ecsDmsReady=true;var COOKIE_NAME='ecs_color_scheme';var ALT_VALUE='alt';function getCurrentScheme(){return document.documentElement.getAttribute('data-ecs-scheme');}
...
function swapLogos(isAlt){var cfg=window.ecsSchemeConfig||{};var darkUrl=cfg.darkLogoUrl;if(!darkUrl){return;}
document.querySelectorAll('.elementor-widget-theme-site-logo img, .elementor-widget-site-logo img, a.custom-logo-link img').forEach(function(img){if(isAlt){if(!img.dataset.ecsLogoDefault){img.dataset.ecsLogoDefault=img.src;}
if(img.src!==darkUrl){img.src=darkUrl;img.srcset='';}}else{if(img.dataset.ecsLogoDefault){img.src=img.dataset.ecsLogoDefault;img.dataset.ecsLogoDefault='';}}});}
...
function init(){syncAllWidgets();swapLogos(getCurrentScheme()===ALT_VALUE);var cfg=window.ecsSchemeConfig||{};if(cfg.systemAuto){var mq=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)');if(mq){mq.addEventListener('change',function(e){if(!hasCookie()){applyScheme(e.matches?ALT_VALUE:'default');syncAllWidgets();}});}}
