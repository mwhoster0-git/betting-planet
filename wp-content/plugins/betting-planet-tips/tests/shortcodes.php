...
bpt_test_assert( ! is_wp_error( $sc_terms ) && 5 === count( $sc_terms ), 'Five shortcode catalog terms required.' );
...
Shortcode_Catalog::install();
bpt_test_assert( 5 === count( get_terms( array( 'taxonomy' => Shortcode_Catalog::TAXONOMY, 'hide_empty' => false ) ) ), 'Catalog initialization duplicated terms.' );
...
$sc_create = static function ( array $record, $status = 'publish', $password = '' ) {
	$meta = array( '_bpt_home_team' => 'Home & United', '_bpt_away_team' => 'Away Town' );
...
do_shortcode( '[bpt_total_tips] [bpt_profit] [bpt_yield] [bpt_win_ratio]' );
